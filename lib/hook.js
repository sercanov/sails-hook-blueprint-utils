...
const defaultTitleBlueprint = function (request, response) {
	...
	_.forEach(model.attributes, function (attribute, attributeName) {
		...
		if (attribute.title) {
			titles[attributeName] = attribute.title
		}
...
const defaultBlueprintFilters = function (request, response) {
	...
	_.forEach(model.attributes, function (attribute, attributeName) {
		...
		if (attribute.filter && !attribute.protected) {

			let filter = {
				name: attributeName,
				text: attribute.title || attributeName
			}
...
const initFn = function (callback) {
	...
	let titlesFn = _.get(sails.middleware, 'blueprints.titles') || defaultTitleBlueprint
...
const routesToAttach = ['count', 'filters', 'associations', 'schema', 'titles', ':id/:model/count']
