 ...
 * @utilsConfig : ( Optional ) Config file that contains array of policies will be applied before hooks.
 * 
 */
...
var utilsConfig;
try { utilsConfig = require('../utils.config.js'); } catch (ex) { }
    ...
    routes: {
      before: {
        'GET /api/:model/count': function (request, response, callback) {
          ...
          if (utilsConfig && utilsConfig.policies && utilsConfig.policies.length > 0)
          {
            utilsConfig.policies.forEach(function(policy){
              ...
              else console.warn('@sails-hook-blueprint-utils : No policy found named -> ' + policy )
            });
