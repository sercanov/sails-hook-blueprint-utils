...
const defaultHeaderBlueprint = function (request, response) {
	...
	_.forEach(model.attributes, function(attribute, attributeName) {
		...
		if (attribute.header) {
			headers[attributeName] = attribute.header
		}
...
const routesToAttach = ['count','filters','associations','schema','headers',':id/:model/count']
